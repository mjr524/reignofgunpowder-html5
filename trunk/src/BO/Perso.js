...
(function (window) {
	...
	this.nextX = 0;  // La prochaine coordonn�e X du perso
	this.nextY = 0; // La prochaine coordonn�e Y du perso
	this.preX = 0; // La pr�c�dente coordonn�e X du perso 
	this.preY = 0;  // La pr�c�dente coordonn�e Y du perso
	...
	this.EnDeplacement = false; // Le d�placement est-il en train de se d�placer?
    ...
    Perso.prototype.initialize = function (imgPerso, position, taille) {
		...
		this.direction = 3; // Direction vers le bas par d�faut
		this.x = position.x;
		this.y = position.y;
		this.nextX = this.x;
		this.nextY = this.y;
	...
	Perso.prototype.Deplacer = function(TypeDeplacement){
		if (!EnDeplacement){
			if (TypeDeplacement == 4){
				...
				this.nextY = this.y;
				this.direction = 4;
				if (Arbre.rectangle.Intersects(this.GetRectangle())==false) // A modifier lors de la cr�ation les d�cors
				{
					EnDeplacement = true;
				}
				else
